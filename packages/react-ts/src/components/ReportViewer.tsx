...
export function ReportViewer() {
  ...
  const diffData = (oldObj: object[], newObj: object[]) => {
    let oldStr = oldObj.map(i => JSON.stringify(i)).sort(alphaSort).join('\n')
    let newStr = newObj.map(i => JSON.stringify(i)).sort(alphaSort).join('\n')
    let raw = jsdiff.diffLines(oldStr, newStr)
    let output = raw.filter(i => i.added || i.removed).map((type, tIndex) => {
  ...
  const handleChange = (result: IParseResult, slug: string, fileName: string) => {
    if (slug === 'next') {
      let sortResult = {
        ...result,
        data: result.data.sort((a, b) => { return a - b })
      }
